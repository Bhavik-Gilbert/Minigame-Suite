...
public class Checkers extends Games {
    ...
    private GridPane displayBoard(ArrayList<ArrayList<Piece>> pieces) {
        ...
        boardPieces.setHgap(this.width/500);
    ...
    private void spaceActions(int spaceListIndex, int spaceRowIndex) {
        ...
        if(pieceListIndex!=null && pieceRowIndex!=null) {
            pieceMove(spaceListIndex, spaceRowIndex, pieceListIndex, pieceRowIndex);
        }

        displayBoard(this.pieces);
    }

    private void pieceMove(int spaceListIndex, int spaceRowIndex, int pieceListIndex , int pieceRowIndex) {
        ...
        if(Math.abs(spaceListIndex-pieceListIndex)==2 && Math.abs(spaceRowIndex-pieceRowIndex)==2) {
            ...
            if(this.pieces.get(killListIndex).get(killRowIndex)!=null && this.pieces.get(killListIndex).get(killRowIndex).getClass()==Piece.class) {
        ...
        if(this.selectedPiece.getType()==TYPE.KING) return;
